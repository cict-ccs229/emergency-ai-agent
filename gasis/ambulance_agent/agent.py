...
class EmergencyVerificationTool(BaseTool):
    ...
    "Try to stay calm, and let me know what happened or if anyone is hurt."
)
            return {
            ...
            verification_message = f"Can you confirm this emergency: '{description}'? Please say 'yes' if it's real or 'no' if it's a mistake."
            return {
...
class ContactAmbulanceTool(BaseTool):
    def call(self, hospital: str, emergency_type: str = "general") -> Dict[str, Any]:
        ...
        if approved:
            ...
            return {
                "status": "success",
                "message": f"✅ An ambulance from {hospital} is now on the way. It should arrive in about {eta_minutes} minutes. Stay where you are and keep calm.",
                "eta": eta_minutes,
                "reference_number": f"EMR-{random.randint(1000, 9999)}",
                "follow_up": "Can you tell me what kind of emergency this is — like an injury, accident, or illness?"
            }
        else:
            fallback_message = f"❌ Ambulance not available at {hospital}. "
            if emergency_state["contact_attempts"] < 3:
                fallback_message += "I’m checking another hospital nearby for you..."
            else:
                fallback_message += "Please call these hotlines for immediate help: Red Cross 143, MMDA 136, or your barangay officials."
